...
__global__ void fdtd_step1_kernel(DATA_TYPE* _fict_, DATA_TYPE *ex, DATA_TYPE *ey, DATA_TYPE *hz, int t, size_t PSIZE)
{
	...
	if ((i < NX) && (j < NY))
	{
		...
		{ 
			ey[i * NY + j] = ey[i * NY + j] - 0.5f*(hz[i * NY + j] - hz[(i-1) * NY + j]) + 0.000001*ex[i*NY+j];
		}
	}
}
...
__global__ void fdtd_step2_kernel(DATA_TYPE *ex, DATA_TYPE *ey, DATA_TYPE *hz, int t, size_t PSIZE)
{
	...
	if ((i < NX) && (j < NY) && (j > 0))
	{
		ex[i * (NY+1) + j] = ex[i * (NY+1) + j] - 0.5f*(hz[i * NY + j] - hz[i * NY + (j-1)]) + 0.0001*ey[i*NY+j];
	}
}
...
int main(int argc, char * argv[])
{
	...
	printf("Start address of fict_gpu:\t%p\n", &(_fict_gpu[0]));
	printf("Start address of ex_gpu:\t%p\n", &(ex_gpu[0]));
	printf("Start address of ey_gpu:\t%p\n", &(ey_gpu[0]));
	printf("Start address of hz_gpu:\t%p\n", &(hz_gpu[0]));

	printf("Size of fict:\t%.2f\n", (float)tmax * sizeof(float) / (1024. * 1024. * 1024.));
    	printf("Size of ex:\t%.2f\n", (float)NX * (NY + 1) * sizeof(int) / (1024. * 1024. * 1024.));
    	printf("Size of ey:\t%.2f\n", (float)NY * (NX + 1) * sizeof(int) / (1024. * 1024. * 1024.));
    	printf("Size of hz:\t%.2f\n", (float)(NX * NY) * sizeof(float) / (1024. * 1024. * 1024.));
