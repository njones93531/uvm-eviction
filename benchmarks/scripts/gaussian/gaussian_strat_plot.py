...
STRATS = ["mmm","mmp","mpm","pmm","mpp","ppm","ppp","pmp","lll","llp","lpl","pll","ppl","plp","lpp","mpl","mlp","lpm","lmp","plm","pml"]
#STRATS = ["mmm","mpp","ppm","ppl","ppp"] 
...
def my_plotter(ax, x, y, xmax, ymax, xlabel, ylabel):
    ...
    for i in range(0, loopMax):
        ...
        ax.plot(x[i], y[i], color=colors[i], marker=MARKERS[i], label=labels[i], alpha=0.8, markersize=10)
