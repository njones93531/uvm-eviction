...
int decompose_field(Settings* settings, Chunk* chunks, int argc, char ** argv);

// Initialise settings from input file
void initialise_application(Chunk** chunks, Settings* settings, int argc, char ** argv)
{
  State* states = NULL;
  read_config(settings, &states);

  //*chunks = (Chunk*)malloc(sizeof(Chunk)*settings->num_chunks_per_rank);
  cudaMallocManaged((void**)chunks, sizeof(Chunk)*settings->num_chunks_per_rank);
...
UM_MAX_MEM = UM_TOT_MEM > UM_MAX_MEM ? UM_TOT_MEM : UM_MAX_MEM;
  ...
  int alloc_index = decompose_field(settings, *chunks, argc, argv);
