...
void initialise_chunk(Chunk* chunk, Settings* settings, int x, int y)
{
  ...
  cudaMallocManaged((void**)&chunk->neighbours, (sizeof(int) * NUM_FACES));
  // Allocate the MPI comm buffers
  int lr_len = chunk->y*settings->halo_depth*NUM_FIELDS;
  cudaMallocManaged((void**)&chunk->left_send, sizeof(double)*lr_len);
  cudaMallocManaged((void**)&chunk->left_recv, sizeof(double)*lr_len);
  cudaMallocManaged((void**)&chunk->right_send, sizeof(double)*lr_len);
  cudaMallocManaged((void**)&chunk->right_recv, sizeof(double)*lr_len);

  int tb_len = chunk->x*settings->halo_depth*NUM_FIELDS;
  cudaMallocManaged((void**)&chunk->top_send, sizeof(double)*tb_len);
  cudaMallocManaged((void**)&chunk->top_recv, sizeof(double)*tb_len);
  cudaMallocManaged((void**)&chunk->bottom_send, sizeof(double)*tb_len);
  cudaMallocManaged((void**)&chunk->bottom_recv, sizeof(double)*tb_len);
...
MEMMAP[chunk->bottom_recv] = sizeof(double) * tb_len;
  ...
  cudaMallocManaged((void**)&chunk->ext, sizeof(ChunkExtension));
