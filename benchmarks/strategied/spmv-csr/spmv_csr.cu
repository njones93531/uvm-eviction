...
//Assumes all memory allocations have same size type (4 bytes)
size_t getN(double psize, double density){
	double a = 3. / (4 * density);
	double b = ((psize/sizeof(float)) - 1.) / (2*density);
...
double getProblemSize(size_t N, size_t nnz){
    ...
    double alloc2 = (double) (N+1) * sizeof(int);
...
// CPU-based SPMV
void cpu_spmv(const size_t num_rows, const size_t num_nonzero,
    ...
    for (size_t i = 0; i < num_rows; ++i) {
        ...
        for (size_t j = row_offsets[i]; j < row_offsets[i + 1]; ++j) {
            ...
            sum += (double)values[j] * (double)x[col];
        }
        y[i] = (float)sum;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 1; i <= N; ++i) {
        h_row_offsets[i] = h_row_offsets[i-1] + (rand() % (nnz/N)) + 1;
    }
    ...
    AccessPolicy acp; 
    acp.setAllocationPolicy((void**) &h_values, nnz * sizeof(float), cudaMemAttachGlobal, 0, argc, argv); 
     acp.setAllocationPolicy((void**) &h_col_indices, nnz * sizeof(int),cudaMemAttachGlobal, 1, argc, argv); 
     acp.setAllocationPolicy((void**) &h_row_offsets, (N + 1) * sizeof(int), cudaMemAttachGlobal, 2, argc, argv); 
     acp.setAllocationPolicy((void**) &h_x, N * sizeof(float), cudaMemAttachGlobal, 3, argc, argv); 
     acp.setAllocationPolicy((void**) &h_y, N * sizeof(float), cudaMemAttachGlobal, 4, argc, argv); 
...
//--------------------------------------------------------------------------
   ...
   if(CPU){
         cpu_spmv(N, nnz, h_values, h_col_indices, h_row_offsets, h_x, h_cpu_y);
        // device result check
        for (size_t i = 0; i < N; i++) {
		double abs_err = fabs(h_y[i] - h_cpu_y[i]);
		double relative_err = abs_err / (double)h_cpu_y[i];
            if (abs_err > ERROR_THRESHOLD) { // direct floating point comparison is not
                correct = 0;            // reliable
    ...
    CHECK_CUDA( cudaFree(h_row_offsets) )
