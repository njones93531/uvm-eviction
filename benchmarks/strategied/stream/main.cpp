...
template <typename T>
void run_triad(int argc, char *argv[]);
...
int main(int argc, char *argv[])
{
  ...
  {
    if (use_float)
      run_triad<float>(argc, argv);
    else
      run_triad<double>(argc, argv);
  }
  else
  {
    if (use_float)
...
void run(int argc, char * argv[])
{
  ...
  stream->init_arrays(startA, startB, startC, argc, argv);
...
void run_triad(int argc, char *argv[])
{
  ...
  std::cout
    << "--------------------------------"
    << std::endl << std::fixed
    << "GPU Runtime: " << std::left << std::setprecision(5) 
    << runtime << "s" << std::endl
    << "Bandwidth (GB/s):  " << std::left << std::setprecision(3)
    << bandwidth << std::endl;
    
  std::cout << "perf," << bandwidth << std::endl;
...
int parseUInt(const char *str, size_t *output)
{
  char *next;
