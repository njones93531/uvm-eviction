...
template <class T>
void CUDAStream<T>::init_arrays(T initA, T initB, T initC, int argc, char * argv[])
{
  //init_kernel<<<array_size/TBSIZE, TBSIZE>>>(d_a, d_b, d_c, initA, initB, initC);
  #pragma omp parallel for simd shared(d_a, d_b, d_c) firstprivate(array_size)
  ...
  acp.setAllocationPolicy((void**)&d_a, array_size*sizeof(T), 0, argc, argv);
  acp.setAllocationPolicy((void**)&d_b, array_size*sizeof(T), 1, argc, argv);
