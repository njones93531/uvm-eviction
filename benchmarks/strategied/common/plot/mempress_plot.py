...
parser.add_argument('fig_dir', type=str, nargs='?', default='../figures/mempress/', help='Directory for saving figures (default: ../figures/mempress/).')
...
df['Sorted Policy'] = df['Policy'].apply(lambda x: ''.join(sorted(x)))
...
grouped_df = df.groupby(['Degree of Subscription', 'Sorted Policy', 'Policy']).agg({'Kernel Time': 'mean'}).reset_index()
grouped_df.loc[grouped_df['Kernel Time']==0,'Kernel Time'] = 10000000 #Mark times that were DNF
grouped_df = grouped_df.groupby(['Degree of Subscription', 'Sorted Policy']).agg({'Kernel Time': 'min'}).reset_index()

#introduce relative performance
#Each psize has its own default time
# Extract the default times where the policy is 'mmm'
default_times = grouped_df[grouped_df['Sorted Policy'] == 'mmm'].set_index('Degree of Subscription')['Kernel Time']

# Map default times to each row based on psize
grouped_df['default_time'] = grouped_df['Degree of Subscription'].map(default_times)



grouped_df = grouped_df[grouped_df['Sorted Policy'].isin(['ddm', 'dmm', 'ddh', 'dhm', 'hmm', 'hhm', 'dhh'])]
grouped_df.reset_index().sort_values("Sorted Policy")


#print("Success")

# Set up positions for each bar cluster
psize_values = np.unique(grouped_df['Degree of Subscription'])
num_clusters = len(grouped_df['Sorted Policy'].unique()) #Num Policies
cluster_width = 0.8 / num_clusters
positions = np.arange(len(psize_values))
time_max = np.max(grouped_df['Kernel Time'])
time_next = np.max(grouped_df[grouped_df['Kernel Time'] != time_max]['Kernel Time'])

# Calculate speedup
grouped_df['Speedup'] = grouped_df['default_time'] / grouped_df['Kernel Time']
