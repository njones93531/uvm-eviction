 ...
 */
template <size_t BLOCK_SIZE> __global__ void MatrixMulCUDA(float *C, float *A,
                                                        float *B, size_t wA,
                                                        size_t wB) {
    // Block index
...
void ConstantInit(float *data, size_t size, const float val) {
    #pragma omp parallel for simd shared(data) firstprivate(val)
...
long MatrixMultiply(int argc, char **argv,
    ...
    float *h_C;/* = reinterpret_cast<float *>(malloc(mem_size_C));*/
        ...
        printf("Allocated Mem: %lfGB\n", (mem_size_A + mem_size_B + mem_size_C) / (double)(1024 * 1024 * 1024.));
    ...
    acp.setAllocationPolicy((void**)&h_A, mem_size_A, 0, argc, argv);
    acp.setAllocationPolicy((void**)&h_B, mem_size_B, 1, argc, argv);
    acp.setAllocationPolicy((void**)&h_C, mem_size_C, 2, argc, argv);
...
//    CUDA_CHECK(cudaMemAdvise(h_A, mem_size_A, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));
//    CUDA_CHECK(cudaMemAdvise(h_B, mem_size_B, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));
//    CUDA_CHECK(cudaMemAdvise(h_C, mem_size_C, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));
