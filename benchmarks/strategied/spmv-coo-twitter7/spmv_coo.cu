...
int main(int argc, char *argv[]) {
    ...
    for (size_t i = 0; i < N; ++i) {
        h_x[i] = (float)rand_r(&seed) / RAND_MAX;
        h_y[i] = 0.0;
    }

    munmap(buffer, fsize);
    AccessPolicy acp;
    acp.setAllocationPolicy((void **)&h_values, nnz * sizeof(float), 0, argc, argv);
    acp.setAllocationPolicy((void **)&h_col_indices, nnz * sizeof(int), 1, argc, argv);
    acp.setAllocationPolicy((void **)&h_row_indices, nnz * sizeof(int), 3, argc, argv);
    acp.setAllocationPolicy((void **)&h_x, N * sizeof(float), 4, argc, argv);
    acp.setAllocationPolicy((void **)&h_y, N * sizeof(float), 5, argc, argv);
    ...
    acp.setAllocationPolicy((void **)&dBuffer, bufferSize, 2, argc, argv);
    clock_t begin = clock();
    // Execute SpMV
