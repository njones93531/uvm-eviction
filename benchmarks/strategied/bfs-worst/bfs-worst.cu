...
int main(int argc, char* argv[]){
    ...
    #pragma omp parallel for simd shared(label) firstprivate(vertices)
	for(uint64_t i = 0; i < vertices; i++){
		label[i] = INF;
	}
	label[startVertex] = 0;
	...
	AccessPolicy acp;
	acp.setAllocationPolicy((void**)&edgeList, sizeof(uint64_t)*2*edges, 0, argc, argv);
	acp.setAllocationPolicy((void**)&vertexList, sizeof(uint64_t)*vertices, 1, argc, argv);
	acp.setAllocationPolicy((void**)&label, sizeof(uint64_t)*vertices, 2, argc, argv);
