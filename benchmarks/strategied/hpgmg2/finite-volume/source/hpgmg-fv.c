...
void bench_hpgmg(mg_type *all_grids, int onLevel, double a, double b, double dtol, double rtol){
     int     doTiming;
     int    minSolves = 1; // do at least minSolves MGSolves
  double timePerSolve = 0;

  //for(doTiming=0;doTiming<=1;doTiming++){ // first pass warms up, second pass times
  for(doTiming=1;doTiming<=1;doTiming++){ // first pass warms up, second pass times
...
//int64_t target_memory_per_rank = -1; // not specified
  ...
  if(argc>=3){
    ...
    char *ptr = argv[1];
    char *tmp;
    size_t target_memory_per_rank = strtol(ptr,&ptr,10);
  ...
  create_level(&level_h,boxes_in_i,box_dim,ghosts,VECTORS_RESERVED,bc,my_rank,num_tasks,NULL, argc, argv);
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  #ifdef USE_HELMHOLTZ
  ...
  mg_type MG_h;
  MGBuild(&MG_h,&level_h,a,b,minCoarseDim, argc, argv);             // build the Multigrid Hierarchy 
