...
void build_interpolation(mg_type *all_grids, int argc, char ** argv){
  int level;
  ...
  if( (level>0) && (all_grids->levels[level]->num_my_boxes>0) ){ // not top  *and*  I have boxes to send
    ...
    for(neighbor=0;neighbor<numFineRanks;neighbor++){
      ...
      for(fineBox=0;fineBox<numFineBoxes;fineBox++)if(fineBoxes[fineBox].recvRank==fineRanks[neighbor]){malloc_size+=elementSize;}
             all_grids->levels[level]->interpolation.send_buffers[neighbor] = (double*)um_malloc(malloc_size*sizeof(double),UM_ACCESS_BOTH, argc, argv);
  ...
  if( (level<all_grids->num_levels-1) && (all_grids->levels[level]->num_my_boxes>0) ){ // not bottom  *and*  I have boxes to receive
    ...
    for(neighbor=0;neighbor<numCoarseRanks;neighbor++){
      ...
      for(coarseBox=0;coarseBox<numCoarseBoxes;coarseBox++)if(coarseBoxes[coarseBox].sendRank==coarseRanks[neighbor]){malloc_size+=elementSize;}
             all_grids->levels[level]->interpolation.recv_buffers[neighbor] = (double*)um_malloc(malloc_size*sizeof(double),UM_ACCESS_BOTH, argc, argv); 
...
void build_restriction(mg_type *all_grids, int restrictionType, int argc, char ** argv){
  int level;
  for(level=0;level<all_grids->num_levels;level++){
  ...
  if( (level<all_grids->num_levels-1) && (all_grids->levels[level]->num_my_boxes>0) ){ // not bottom  *and*  I have boxes to send
    ...
    for(neighbor=0;neighbor<numCoarseRanks;neighbor++){
      ...
      for(coarseBox=0;coarseBox<numCoarseBoxes;coarseBox++)if(coarseBoxes[coarseBox].recvRank==coarseRanks[neighbor]){malloc_size+=elementSize;}
             all_grids->levels[level]->restriction[restrictionType].send_buffers[neighbor] = (double*)um_malloc(malloc_size*sizeof(double),UM_ACCESS_BOTH, argc, argv);
  ...
  if( (level>0) && (all_grids->levels[level]->num_my_boxes>0) ){ // not top  *and*  I have boxes to receive
    ...
    for(neighbor=0;neighbor<numFineRanks;neighbor++){
      ...
      for(fineBox=0;fineBox<numFineBoxesRemote;fineBox++)if(fineBoxes[fineBox].sendRank==fineRanks[neighbor]){malloc_size+=elementSize;}
             all_grids->levels[level]->restriction[restrictionType].recv_buffers[neighbor] = (double*)um_malloc(malloc_size*sizeof(double),UM_ACCESS_BOTH, argc, argv);
...
void MGBuild(mg_type *all_grids, level_type *fine_grid, double a, double b, int minCoarseGridDim, int argc, char** argv){
...
//if(fine_grid->dim.k<coarse_dim)coarse_dim = fine_grid->dim.k;
  ...
  for(level=1;level<all_grids->num_levels;level++){
    ...
    create_level(all_grids->levels[level],boxes_in_i[level],box_dim[level],box_ghosts[level],all_grids->levels[level-1]->numVectors,all_grids->levels[level-1]->boundary_condition.type,all_grids->levels[level-1]->my_rank,nProcs[level],all_grids->levels[level-1], argc, argv);
    all_grids->levels[level]->h = 2.0*all_grids->levels[level-1]->h;
  }


  // bottom solver (level = all_grids->num_levels-1) gets extra vectors...
  create_vectors(all_grids->levels[all_grids->num_levels-1],all_grids->levels[all_grids->num_levels-1]->numVectors + IterativeSolver_NumVectors() , argc, argv);
  ...
  build_restriction(all_grids,RESTRICT_CELL  , argc, argv); // cell-centered
  build_restriction(all_grids,RESTRICT_FACE_I, argc, argv); // face-centered, normal to i
  build_restriction(all_grids,RESTRICT_FACE_J, argc, argv); // face-centered, normal to j
  build_restriction(all_grids,RESTRICT_FACE_K, argc, argv); // face-centered, normal to k
  build_interpolation(all_grids, argc, argv);
...
//------------------------------------------------------------------------------------------------------------------------------
void MGPCG(mg_type *all_grids, int onLevel, int x_id, int F_id, double a, double b, double dtol, double rtol, int argc, char ** argv){
  ...
  for(l=0;l<all_grids->num_levels;l++){
    create_vectors(all_grids->levels[l],VECTORS_RESERVED+3, argc, argv);
  }
