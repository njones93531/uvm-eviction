...
T *dd_a, *dd_b, *dd_c;
//cudaMalloc(reinterpret_cast<void **>(&dd_a), ARRAY_SIZE*sizeof(T));
cudaMalloc(reinterpret_cast<void **>(&dd_b), ARRAY_SIZE*sizeof(T));
cudaMalloc(reinterpret_cast<void **>(&dd_c), ARRAY_SIZE*sizeof(T));

std::cout << "alloced," << ARRAY_SIZE*sizeof(T) * 3 << std::endl; 
cudaMemAdvise(d_a, array_size, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId);
//cudaMemAdvise(d_b, array_size, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId);
//cudaMemAdvise(d_c, array_size, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId);
//cudaMemAdvise(d_a, array_size, cudaMemAdviseSetAccessedBy, 0);
//cudaMemAdvise(d_a, array_size, cudaMemAdviseSetReadMostly, 0);
 cudaMemcpy(dd_b, d_b, ARRAY_SIZE*sizeof(T), cudaMemcpyHostToDevice);
 cudaMemcpy(dd_c, d_c, ARRAY_SIZE*sizeof(T), cudaMemcpyHostToDevice);
// cudaMemcpy(d_c, dd_a, ARRAY_SIZE*sizeof(T), cudaMemcpyHostToDevice);
}
