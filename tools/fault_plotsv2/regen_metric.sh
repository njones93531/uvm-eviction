...
for size_GB in 60 120 240; do
  ...
  for log in $(find ../../benchmarks/tyler-default-complex/ -name '*_klog'); do
      found=false
      ...
      if [ "$found" != true ]; then
        ...
        if [ ! -e $output_file ]; then
            ...
            job_id=$(sbatch --parsable --job-name=$job_name --output=$slurm_file --partition=hsw --time=4:00:00 --mem=${size_GB}G --cpus-per-task=4 --wrap="ulimit -v ${size_KB}; python3 metric_plot.py $log")
            job_ids+=($job_id)
        else
