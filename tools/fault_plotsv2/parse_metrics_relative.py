...
def print_paper_mets(data_df):
    ...
    data_df.to_csv("simple_paper_metrics.csv", index=False)
...
def print_strat_naive(data_df, pf, p=False):
    pivots = []
    strat_names = ['Migrate', 'Pin To Host', 'DM', 'DH', 'Predicted', 'Opt']
    ...
    print(reshape_pivot_tables(pivots, strat_names))
...
def make_accuracy_plot(csv_file):
    ...
    df_counts = pd.DataFrame(
        {t: (df[numeric_cols] < t).sum() / 16 for t in thresholds}
    )
    ...
    plt.plot([0.127], [1], color=colors[4], marker='x', markersize=16)
...
if __name__ == "__main__":
    ...
    full_df = parse_df()
    pf = parse_perf_df()
    #make_accuracy_plot("/home/najones/uvm-eviction/tools/fault_plotsv2/NaiveData1.csv")
    #make_perf_barchart("/home/najones/uvm-eviction/tools/fault_plotsv2/NaiveData1.csv")
    #make_all_perf_barchart("/home/najones/uvm-eviction/tools/fault_plotsv2/NaiveData1.csv")
    #print_strat_naive(full_df, pf, p=True)
    radar_plot(full_df.copy())
    exit()
    #print_strat_naive(full_df, pf)
    classify_apps_hardcode(full_df)
    predict_strat_hardcode(full_df.copy(), pf.copy())
    #print_paper_mets(full_df.copy())
    exit()
    ...
    print("Ranking d classifiers")
    d_dict, brute_force_best, best_mets = brute_force_metrics_vs_d(df1.copy(), metrics=[], perf_df=pf.copy(), p=False)
