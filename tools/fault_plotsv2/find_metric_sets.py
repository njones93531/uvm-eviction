...
def print_x_classification(data_df, metric):
    print("Which allocations appear to belong to FDTD or stream?")
    ...
    for idx, group in groups:
        ...
        for loc, c in enumerate(strategy):
            if tmp['predict_x'][loc]:
                strategy[loc] = 'x'

        data_df.loc[(data_df['app'] == idx[0]) & (data_df['psize'] == idx[1]), 'x_class'] = str(''.join(strategy))
    ...
    for subdir, _, files in os.walk(main_directory):
        ...
        for file in files:
            ...
            # Add a new columns
                ...
                data_df['strat'] = list(solution[psize][app]) + [None]
                ...
                if enable_good_OR:
                    for metric1, metric2 in [('d_prop_max_1000', 'size'), ('tr_mean_1000', 'tr_prop_0s_1')]:
...
def find_border_x(data_df, metric):
    ...
    scores = {}
    values = sorted(met_strat.keys())
    num_x = list(met_strat.values()).count('m')
...
def rank_m_h_classifiers(data_df, p=False):
    ...
    metrics_to_tuples = {}
    for metric in data_df.columns:
        if numeric(data_df[metric].iloc[0]) and metric in cols:
...
def score_x(data_df):
    score = 2 * (data_df['predict_x'] & data_df['x']).sum()
    score += (data_df['predict_x'] == data_df['x']).sum()
    return score

def predict_x(data_df, metric_tuple, p=False):
    metric, thold, x_is_up = metric_tuple
...
def classify_d_hard_code(data_df, p=False, apply_m=False, perf_df=pd.DataFrame()):
    ...
    fdtd_stream_view.loc['new'] = fdtd_stream_view['d_prop_max_1000_OR_size'] * -1
...
def brute_force_metrics_vs_d(data_df, metrics=[], perf_df=pd.DataFrame(), p=False):
    ...
    for i, classifier in enumerate(classifiers):
        ...
        if numeric(df[classifier].iloc[0]) and df[classifier].nunique() != 1: 
            for m in [-1, 1]:
                for invert in [False]: #[True, False]
                    for mode in ["knapsack"]: #["knapsack", "migrate"]:
                        score = classify_d_with(data_df, classifier, m, mode, False, invert, False, perf_df)
...
def classify_both(df_data, df_perf, d_predictor, m_predictors, p=False):
    set_m_prediction(df_data, m_predictors, p)
    return classify_d_with(df_data, d_predictor[0], d_predictor[1], d_predictor[2], p, invert=d_predictor[3], apply_m=True, perf_df=df_perf)
...
def try_fdtd_vs_stream(df, best_x_metrics):
    xmets = [find_border_x(df.copy(), met) for met in best_x_metrics]
    scores_to_xmets = {}
    max_score = 0
    for met in xmets:
        ...
        if score > max_score:
            max_score = score
    for met in scores_to_xmets[max_score]:
