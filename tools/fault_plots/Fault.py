...
#        print("avg faults per utlb:", avgs)
    ...
    def print_batch_time_info(self):
        print("avg batch time ns", self.avg_time_batches())
        print("max batch time ns", max(self.batch_times))
        print("min batch time ns", min(self.batch_times))
        print("stddev batch time", stdev(self.batch_times))

    def print_avg_bandwidth(self):
        NS_PER_S = 1000000000
        KB_PER_GB = 1024 * 1024
        #Duplicate faults do not transfer data 
        dups_4k = sum(self.get_duplicate_faults_4k()) 

        real_faults = len(self.faults) - dups_4k
        data_transfer_GB = 4 * real_faults / KB_PER_GB
        total_batch_time = sum(self.batch_times) / NS_PER_S
        bandwidth = data_transfer_GB / total_batch_time
        print("data transferred GB", data_transfer_GB)
        print("total batch time s", total_batch_time)
        print("avg bandwidth GB/s", bandwidth) 
    ...
    def print_info(self):
        ...
        self.print_avg_bandwidth()
        self.print_batch_time_info()
