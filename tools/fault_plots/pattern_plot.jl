...
function build_dataset(inf, t)
    ...
    return haddrs, maddrs, paddrs, eaddrs, hx, mx, px, ex, ranges, maximum(fulladdrs), max_x
...
function main()
    ...
    #    @time faddrs, fx, ranges = build_dataset(inf, time)
        @time haddrs, maddrs, paddrs, eaddrs, hx, mx, px, ex, ranges, max_y, max_x = build_dataset(inf, time) #, STATS_ONLY)
		...
		xformatter = x -> string(round(Int, x / floor(log10(abs(max_x))))),
             	yformatter = y -> string(round(Int, y / floor(log10(abs(max_y))))))

        GC.enable(true)
        println(stderr, "hlines")
        GC.enable(false)
	if isempty(title)
            title=string(rsplit(inf, '/')[6])
        ...
        if time
         ...
         #markerstrokewidth=0,
                    color = :green,
         #label = "Faults",
                    yformatter = y -> string(y / 10^floor(log10(abs(max_y)))),
                    xformatter = x -> string(x / 10^floor(log10(abs(max_x)))),
                    grid = false,
        ...
        if !isempty(px)
            @time p = scatter!(px, paddrs, color=:blue, markersize=markersize, markerstrokewidth=0, label="Prefetches")
           println(stderr, "prefetches")
        end
    ...
    annotate!([(0, max_y * 1.05, Plots.text(string("e",floor(Int, log10(abs(max_y)))), 11, :black, :center))])
    annotate!([(max_x * 1.002, 0, Plots.text(string("e", floor(Int, log10(abs(max_x)))), 11, :black, :center))])
